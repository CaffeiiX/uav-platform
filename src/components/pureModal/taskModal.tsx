...
const TaskModal: React.FC<{}> = ({}) => {
  ...
  const [platformPointCol, setPlatformPointCol] = useRecoilState(platformPointColAtom);
  ...
  const onButtonOk = async () => {
    ...
    if (methodMode === "1") {
      setEntitiesProperties({
        ...
        entitiesProperties: entitiesProperties.entitiesProperties.map((item) =>
          item.key === "taskBoundaryComponent"
            ? { ...item, visual: true }
            : {
                ...item,
                visual: false,
    ...
    if (methodMode === "2" || methodMode === "3") {
      const polygonRegionDegreesList = polygonRegion.map((item) =>
      ...
      const [uavArea, uavBoundary] = calculatePolygonVoroniArea(
        polygonRegionDegreesList,
        uavPointList,
        selectUavList
      );
      ...
      setEntitiesProperties({
        ...
        entitiesProperties: entitiesProperties.entitiesProperties.map((item) =>
          item.key === "taskBoundaryComponent" ||
          item.key === "platformPointsComponent" ||
          item.key === "viewUavVisualColComponent" ||
          item.key === "selectPlanAreaComponent"
            ? { ...item, visual: true }
            : {
                ...item,
                visual: false,
    ...
    if (methodMode === "4") {
      ...
      const targetPointList = targetPointCol.map((item) =>
        Cartesian3ToDegrees(item)
      );
      const polygonRegionList = polygonRegion.map((item) =>
        Cartesian3ToDegrees(item)
      );
      const uavPointListNew = getUavPointList(
        platformSelectUavList,
        platformPointCol
      ).map((item) => Cartesian3ToDegrees(item));
      ...
      setEntitiesProperties({
        ...
        entitiesProperties: entitiesProperties.entitiesProperties.map((item) =>
          item.key === "taskBoundaryComponent" ||
          item.key === "platformPointsComponent" ||
          item.key === "targetPointsComponent" 
            ? { ...item, visual: true }
            : {
                ...item,
                visual: false,
